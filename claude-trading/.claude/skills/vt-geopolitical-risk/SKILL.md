...
```python
# Option A: Use the Jina Reader API integrated in the project through WebFetch
def analyze_geopolitical_news(query: str) -> dict:
    """Read news through Jina and analyze geopolitical-risk sentiment.

    Use together with the agent's WebFetch tool.
    ...
    # Use WebFetch to fetch content, then pass it to the LLM to extract risk events
